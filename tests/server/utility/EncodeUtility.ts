import type { FileWriteArgs, FileReadArgs, AssetObject, AssetObjects, AudibleInstanceObject, ClipObject, DecodeArgs, DecodeOptions, Decoding, DropResource, EncodeArgs, EndpointRequest, InstanceObject, JobOptions, MashAssetObject, OutputOptions, Point, ShapeAssetObject, Size, StringDataOrError, TextInstanceObject, TrackObject, VideoOutputOptions, VisibleInstanceObject } from '@moviemasher/shared-lib/types.js'
import type { Command } from '../../../packages/@moviemasher/server-lib/src/type/Command.js'

import { $WRITE, $CONTAINER, $CROP, $CSS, $CUSTOM, $DECODE, $DURATION, $ENCODE, $IMAGE, $LONGEST, $LUMINANCE, $MASH, $NONE, $PROBE, $TTF, $TXT, $VIDEO, DASH, DEFAULT_CONTAINER_ID, DEFAULT_CONTENT_ID, DIRECTIONS_SIDE, DOT, DURATION_UNKNOWN, ERROR, MOVIE_MASHER, SIZE_OUTPUT, arrayUnique, errorCaught, errorPromise, errorThrow, idReset, isDefiniteError, namedError, typeOutputOptions } from '@moviemasher/shared-lib/runtime.js'
import path from 'path'
import { ffmpegCommand } from '../../../packages/@moviemasher/server-lib/src/command/CommandFactory.js'
import { $DirRoot, $OutputRoot, ENV,  } from '../../../packages/@moviemasher/server-lib/src/utility/env.js'
import { directoryCreatePromise, fileMovePromise, fileNameFromOptions, filePathExists, readFileFunction, removeFilePromise, writeFileFunction, filenameAppend } from '../../../packages/@moviemasher/server-lib/src/module/file.js'
...
import { assertAbsolutePath, assertDefined, assertPoint, assertSize, isPoint, isSize } from '@moviemasher/shared-lib/utility/guards.js'
import { stringSeconds } from '@moviemasher/shared-lib/utility/time.js'

const commandPromise = async (command: Command) => {
...
export const TestDirTemporary = ENV.get($OutputRoot)
const TestDirSnapshots = path.resolve(ENV.get($DirRoot), 'tests/server/snapshots')
...
export const GenerateAssetObjects: AssetObjects = [
  {
    ...
    source: 'raw',
    type: "image",
    resources: [{ type: $IMAGE, request: { endpoint: { pathname: "/moviemasher/temporary/assets/image/puppy/image.jpg" } } }],
  ...
  } as AssetObject,
  {
    ...
    source: 'raw',
    type: "video",
    resources: [{ type: $VIDEO, request: { endpoint: { pathname: "/moviemasher/temporary/assets/video/rgb.mp4" } } }],
  ...
  } as AssetObject,
  {
    ...
    source: 'raw',
    type: "image",
    resources: [{ type: $IMAGE, request: { endpoint: { pathname: "/moviemasher/temporary/assets/image/kitten/image.jpg" } } }],
  ...
  } as AssetObject,
  {
    ...
    type: $IMAGE,
    label: "Lobster",
    resources: [{ type: $TTF, request: { endpoint: "/moviemasher/temporary/assets/font/lobster/lobster.ttf" } }],
  } as AssetObject,
  {
    id: "font.valken",
    source: 'text',
    type: $IMAGE,
    label: "Valken",
    resources: [{ type: $TTF, request: { endpoint: "/moviemasher/temporary/assets/font/valken/valken.ttf" } }],
  } as AssetObject,
  // same size as text ^
  { 
    "label": "Text Rect",
  ...
  } as ShapeAssetObject,
  {
    ...
    label: "Video", 
    id: "video",
    resources: [{ type: $VIDEO, request: { endpoint: { pathname: "/moviemasher/temporary/assets/video/dance.mp4" } } }],
  } as AssetObject,
  {
...
const checkSnapshot = async (id: string, outputPath: string): Promise<StringDataOrError> => {
  ...
  const fileArgs: FileReadArgs = { path: snapshotPath }
  const fileOrError = await readFileFunction(fileArgs)
  ...
  const writtenOrError = await readFileFunction(fileArgs)
  ...
  const previousPath = path.join(TestDirSnapshots, [id, 'prev', $TXT].join(DOT))
  assertAbsolutePath(previousPath)

  const fileArgs: FileWriteArgs = { path: previousPath, content: existingSnapshot, type: $WRITE }
  await writeFileFunction(, existingSnapshot)
  // return { data: 'OK' }
...
export const encodeId = async (id: string, outputOptions: OutputOptions, duration = DURATION_UNKNOWN, labels = false) => {
  ...
  if (encodedPreviously) {
    if (filePathExists(previousPath)) await removeFilePromise(previousPath)
  ...
  } else if (encodedPreviously) await removeFilePromise(previousPath)
  return { data: 'OK' }
}
...
export const combineIds = async (ids: GenerateTestIds, id:string): Promise<StringDataOrError> => {
  ...
  const srtOrError = await writeFileFunction(srtPath, srtString)
