...
describe('Time', () => {
  ...
  describe('timeFromArgs', () => {
    test('throws when frame is invalid', () => {
      const frameString = '0' as unknown as number
      assert.throws(() => timeFromArgs(frameString, 1), 'frameString')
      assert.throws(() => timeFromArgs(-1, 1), 'negative frame')
      // assert.throws(() => timeFromArgs(0.1, 1), 'float frame')
    })
    test('throws when fps is invalid', () => {
...
describe('stringSeconds', () => {
  test('returns expected response', () => {
    assert.equal(stringSeconds(0, 10, 0), '0.0')
