...
import { Is, Context, Throw, Time } from "../Utilities"
...
const clipCanBeSplit = (clip : Clip, time : Time, quantize : number) : boolean => {
...
class Masher extends Base {
  ...
  add(object, addType, frameOrIndex, trackIndex) {
    ...
    const media = MediaFactory.createFromObject({ type, ...object })
