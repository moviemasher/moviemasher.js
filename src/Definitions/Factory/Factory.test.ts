...
import { VideoClassImplementation } from "../../Mash/Video/VideoInstance"
...
import { VideoSequenceClassImplementation } from "../../Mash/VideoSequence/VideoSequenceInstance"
import { VideoSequenceDefinitionClassImplementation } from "../../Mash/VideoSequence/VideoSequenceDefinition"

describe("Factory", () => {
  const dimensions = { width: 640, height: 480 }
  Cache.visibleContext.size = dimensions
  ...
  describe(ClipType.Video, () => {
    ...
    describe("instance", () => {
      test("returns VideoClassImplementation instance", () => {
        expect(videoDefinition().instance).toBeInstanceOf(VideoClassImplementation)
      })
    })
    // describe("definition", () => {
  ...
  describe(ClipType.VideoSequence, () => {
    ...
    describe("definition", () => {
       test("returns VideoSequenceDefinitionClassImplementation instance", () => {
        expect(definition()).toBeInstanceOf(VideoSequenceDefinitionClassImplementation)
      })
    //   test("returns instance whose urls reflect duration", () => {
    ...
    describe("instance", () => {
      test("returns VideoClassImplementation instance", () => {
        expect(definition().instance).toBeInstanceOf(VideoSequenceClassImplementation)
      })
    })
  ...
  describe("image", () => {
    ...
    //   test("returns expected command", async () => {
    //     const time = Time.fromArgs(0, 1)
    //     const clip = imageDefinition().instanceFromObject({ frames: 1 })
    //     await clip.loadClip(time.fps, time)
    //     const command = clip.inputCommandAtTimeToSize(time, time.fps, dimensions)
    //     expect(command).toBeDefined()
    //     if (!command) throw Errors.internal

    //     // console.log(command)
    //     expect(command.sources?.length).toBeGreaterThan(0)
    //   })
    // })
