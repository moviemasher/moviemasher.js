                                ...
                                int *got_frame, AVPacket *pkt)
{
    ...
    *got_frame = 0;
    ...
    if ((s->base_uri) && (strlen(s->base_uri)>0)) {
        file = g_file_new_for_uri(s->base_uri);
        stream = g_memory_input_stream_new_from_data(pkt->data, pkt->size, NULL);
...
static const AVOption options[] = {
    ...
    { "base_uri", "Absolute path above referenced files", OFFSET(base_uri), AV_OPT_TYPE_STRING, { .str = "" }, 0, 0, DEC },
    { NULL },
};
