...
import { ExpressHandler } from '../Server.js'
import { HostServers } from '../../Host/Host.js'
import { DataServer, DataServerArgs } from './DataServer.js'
import { FileServer } from '../FileServer/FileServer.js'
import { idUnique } from '../../Utilities/Id.js'
import { RenderingServer } from '../RenderingServer/RenderingServer.js'
import { EnvironmentKeyAppColumnOwner, EnvironmentKeyAppColumnSource } from '@moviemasher/lib-server'
import { AssetObject, AssetObjects, ErrorName, Identified, JsonRecord, MashAssetObject, PotentialError, StringRecord, TypeVideo, UnknownRecord, errorCaught, errorName } from '@moviemasher/runtime-shared'
import { DataDefinitionDeleteRequest, DataDefinitionDeleteResponse, DataDefinitionGetRequest, DataDefinitionGetResponse, DataDefinitionPutRequest, DataDefinitionPutResponse, DataDefinitionRetrieveRequest, DataDefinitionRetrieveResponse, DataGetRequest, DataMashDefaultRequest, DataMashDefaultResponse, DataMashDeleteRequest, DataMashDeleteResponse, DataMashGetResponse, DataMashPutRequest, DataMashPutResponse, DataMashRetrieveRequest, DataRetrieveResponse, DataServerInit, DescribedObject } from '../../Api/Data.js'
...
const DataServerInsertRecord = (userId: string, data: any): StringRecord => {
...
export class DataServerClass extends ServerClass implements DataServer {
  ...
  defaultMash: ExpressHandler<DataMashDefaultResponse | PotentialError, DataMashDefaultRequest> = async (req, res) => {
    ...
    const response: DataMashDefaultResponse = { mash: { type: TypeVideo, source: TypeMash, id: idTemporary(), media: [] }, previewSize }
    try {
  ...
  getMash: ExpressHandler<DataMashGetResponse, DataGetRequest> = async (req, res) => {
    const { id } = req.body
    const response: DataMashGetResponse = { mash: { type: TypeVideo, source: TypeMash, id: idTemporary(), media: [] } }
    try {
      const user = this.userFromRequest(req)
  ...
  private insertDefinitionPromise(userId: string, definition: AssetObject): Promise<string> {
  ...
  private mashInsertPromise(userId: string, mash: MashAssetObject, assetIds?: string[]): Promise<StringRecord> {
  ...
  private mashUpdatePromise(userId: string, mash: MashAssetObject, assetIds?: string[]): Promise<StringRecord> {
    const { id, label, ...rest } = mash
    if (!id) return Promise.reject(401)

    const json = JSON.stringify(rest)
    const data = { userId, id, label, json }
  ...
  private selectDefinitionsPromise(userId: string, types: string[], columns = DataServerColumnsDefault): Promise<AssetObject[]> {
  ...
  private selectMashRelationsPromise(mashId: string): Promise<AssetObjects> {
    return this.database
      .select('definition.*')
  ...
  private updateDefinitionPromise(definition: AssetObject): Promise<void> {
    const { type, id, label, ...rest } = definition
    if (!id) return Promise.reject(401)

    const json = JSON.stringify(rest)
    const data = { id, label, json }
  ...
  private writeDefinitionPromise(userId: string, definition: AssetObject): Promise<string> {
    const { id } = definition
  ...
  private writeMashPromise(userId: string, mash: MashAssetObject, assetIds?: string[]): Promise<StringRecord> {
    const { id } = mash
