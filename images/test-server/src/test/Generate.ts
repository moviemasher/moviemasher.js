import type { AssetObjects, ClipObject, ColorInstanceObject, ImageAssetObject, InstanceObject, MashAssetObject, Point, ShapeAssetObject, Size, TextAssetObject, TextInstanceObject, TrackObject, VideoAssetObject } from '@moviemasher/runtime-shared'

import { DIRECTIONS, DefaultContainerId, DefaultContentId, DurationUnknown, TextHeight, assertPoint, assertPopulatedArray, assertPopulatedString, assertSize, colorBlack, colorBlue, colorRed, isInstanceObject, isPoint, isPopulatedArray, isSize, sizeScale } from '@moviemasher/lib-shared'
import { IMAGE, SIZE_OUTPUT, SourceMash, TypeFont, VIDEO, errorThrow, isPopulatedString, isString } from '@moviemasher/runtime-shared'
...
const textOptions: TextInstanceObject = { 
  string: "Butcherman",
  assetId: "com.moviemasher.font.butcherman"
}
...
export const generateTest = (testId: GenerateTestId, size = SizePreview, frames = DurationUnknown, labels = false): GenerateMashTest => {
  ...
  const textAssetObject: TextAssetObject = { 
    "label": "Butcherman",
  ...
  const mash: MashAssetObject = { 
    ...
    source: SourceMash,
    id: testId, color: '#666666', tracks,
    assets: [ textAssetObject, ...GenerateAssetObjects ],
  }
  return [testId, mash]
}
