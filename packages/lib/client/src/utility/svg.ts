...
import { assertDefined, assertPoint, assertPopulatedString, assertTrue, isPoint, isPositive, isSize } from '@moviemasher/lib-shared/utility/guards.js'
import { assertSizeAboveZero, pointValueString, rectTransformAttribute, sizeAboveZero, sizeCopy } from '@moviemasher/lib-shared/utility/rect.js'
import { CURRENT_COLOR, HEIGHT, NAMESPACE_SVG, NONE, POINT_ZERO, SHORTEST, WIDTH, idGenerateString, isArray, isPopulatedString } from '@moviemasher/runtime-shared'
...
export const svgGroupElement = (dimensions?: any, id = ''): SVGGElement => {
  const element = globalThis.document.createElementNS(NAMESPACE_SVG, 'g')
  svgSet(element, id)
...
export const svgPolygonElement = (dimensions: any, className?: string | string[], fill = '', id?: string): SVGPolygonElement => {
  const element = globalThis.document.createElementNS(NAMESPACE_SVG, 'polygon')
...
export const svgSvgElement = (size?: Size, svgItems?: SvgItem | SvgItems): SVGSVGElement => {
  const element = globalThis.document.createElementNS(NAMESPACE_SVG, 'svg')
...
export const svgImageElement = () => {
  const element = globalThis.document.createElementNS(NAMESPACE_SVG, 'image')
  svgSet(element, NONE, 'preserveAspectRatio')
  return element
}

export const svgPathElement = (path: string, fill = CURRENT_COLOR) => {
  const element = globalThis.document.createElementNS(NAMESPACE_SVG, 'path')
...
export const svgMaskElement = (size?: Size, contentItem?: SvgItem, luminance?: boolean): SVGMaskElement => {
  ...
  const element = globalThis.document.createElementNS(NAMESPACE_SVG, 'mask')
  svgSet(element, id)
...
export const svgClipPathElement = (size?: Size, contentItem?: SvgItem): SVGClipPathElement => {
  const id = idGenerateString()
  const element = globalThis.document.createElementNS(NAMESPACE_SVG, 'clipPath')
  svgSet(element, id)
  if (sizeAboveZero(size)) {
...
export const svgFilter = (values: StringRecord, dimensions?: any): SvgFilter => {
  ...
  const element = globalThis.document.createElementNS(NAMESPACE_SVG, filter) 
  svgSetDimensions(element, dimensions)
...
export const svgPatternElement = (dimensions: Size, id: string, items?: SvgItem | SvgItems): SVGPatternElement => {
  const element = globalThis.document.createElementNS(NAMESPACE_SVG, 'pattern')
  svgSet(element, id)
...
export const svgDefsElement = (svgItems?: SvgItems): SVGDefsElement => {
  const element = globalThis.document.createElementNS(NAMESPACE_SVG, 'defs') 
  svgAppend(element, svgItems)
  return element
}

export const svgFeImageElement = (id?: string, result?: string): SVGFEImageElement => {
  const element = globalThis.document.createElementNS(NAMESPACE_SVG, 'feImage') 
...
export const svgFilterElement = (filters?: SvgFilters, filtered?: SvgItem | SvgItems, rect?: any, units = 'userSpaceOnUse'): SVGFilterElement => {
  const filterElement = globalThis.document.createElementNS(NAMESPACE_SVG, 'filter')
...
export const svgUseElement = (href?: string, className?: string, id?: string) => {
  const element = globalThis.document.createElementNS(NAMESPACE_SVG, 'use')
...
export const svgFunc = (type: string, values: string) => {
  const element = globalThis.document.createElementNS(NAMESPACE_SVG, type)
...
export const svgText = (string: string, family: string, size: number, transform: string, color?: string) => {
  const svgItem = globalThis.document.createElementNS(NAMESPACE_SVG, 'text')
