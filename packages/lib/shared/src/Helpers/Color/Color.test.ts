...
import { colorHexToRgb, colorMixRbg, colorRgbaToHex, colorToRgb, colorValid } from './ColorFunctions.js'
...
describe('Color', () => {
  ...
  describe('colorRgbaToHex', () => {
    ...
    Object.entries(colors).forEach(([color, value]) => {
      test(`rgbaToHex('${color}')`, () => {
        assert.deepStrictEqual(colorRgbaToHex(value), color)
      })
    })
  })
  ...
  describe('colorToRgb', () => {
    ...
    Object.entries(colors).forEach(([color, value]) => {
      test(`toRgb('${color}')`, () => {
        assert.deepStrictEqual(colorToRgb(color), value)
      })
    })
  })
})
