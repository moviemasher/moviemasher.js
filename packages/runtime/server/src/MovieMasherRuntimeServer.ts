...
export const MOVIEMASHER_SERVER: MovieMasherServerRuntime = {
  eventDispatcher: new ServerEventDispatcher(),
  options: {
    imports: {
      [COLOR]: '@moviemasher/lib-server/asset/color/image.js',
  ...
  get importPromise() { 
    const { options, eventDispatcher } = MOVIEMASHER_SERVER
    const { imports } = options
    return importPromise(imports, eventDispatcher, 'Server')
  },
}
