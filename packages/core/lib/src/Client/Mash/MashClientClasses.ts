import { AssetCacheArgs, CacheOptions, Component, InstanceCacheArgs, PreloadArgs, PreloadOptions } from '../../Base/Code.js'
...
import { ClientClip, ClientClips, ClientTrack, ClientTracks, MashClientAsset, MashClientAssetObject } from './MashClientTypes.js'
...
import { Interval } from '../../Shared/Mash/MashTypes.js'
...
import { trackInstance } from '../../Shared/Mash/Track/TrackFactory.js'
...
import { pixelToFrame } from '../../Utility/PixelFunctions.js'
import { assertClientVisibleInstance } from '../ClientGuards.js'
import { MovieMasher } from '@moviemasher/runtime-client'

export type TrackClips = [number, ClientClips]

export class MashClientAssetClass extends MashAssetClass implements MashClientAsset {
  constructor(object: MashClientAssetObject) {
    super(object)
    const { editor, loop, buffer } = object
    this.editor = editor
  ...
  addTrack(options: TrackObject = {}): Track {
    ...
    const track = trackInstance(args) as ClientTrack
    this.tracks.push(track)
    this.tracks.sort(sortByIndex)
    MovieMasher.dispatch(EventTypeTrack)
    return track
  }
  ...
  private compositeVisible(time: Time): void {
    ...
    this.clearPreview()
    MovieMasher.dispatch(EventTypeDraw)
  }
  ...
  private drawTime(time: Time): void {
    ...
    this.drawnTime = time
    this.drawRequest()
    MovieMasher.dispatch(timeChange ? EventTypeTime : EventTypeLoaded)
  }
  ...
  private emitIfFramesChange(method: () => void): void {
    ...
    if (origFrames !== frames) {
      MovieMasher.dispatch(EventTypeDuration)
  ...
  private handleDrawInterval(): void {
    ...
    if (seconds >= this.endTime.seconds) {
      ...
      if (this.loop) this.seekToTime(this.time.withFrame(0))
        ...
        this.paused = true
        MovieMasher.dispatch(EventTypeEnded)
      }
    } else {
  ...
  set paused(value: boolean) {
    ...
    if (paused) {
      ...
      MovieMasher.dispatch(EventTypePause)
    } else {
      ...
      // console.log('Mash emit', EventTypePlay)
        MovieMasher.dispatch(EventTypePlay)
      })
    }
  }
  ...
  private set playing(value: boolean) {
    ...
    if (this._playing !== value) {
      ...
      if (value) {
        ...
        MovieMasher.dispatch(EventTypePlaying)
  ...
  private restartAfterStop(time: Time, paused: boolean, seeking?: boolean): void {
    if (time.equalsTime(this.time)) { // otherwise we must have gotten a seek call
      if (seeking) {
        ...
        MovieMasher.dispatch(EventTypeSeeked)
      }
      this.drawTime(time)
  ...
  seekToTime(time: Time): Promise<void> | undefined {
    if (this.seekTime !== time) {
      this.seekTime = time
      MovieMasher.dispatch(EventTypeSeeking)
      MovieMasher.dispatch(EventTypeTime)
    }
