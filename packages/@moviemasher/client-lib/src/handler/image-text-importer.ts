...
import { EventImporterNodeFunction, EventImport, EventImporterAdd, EventImporterError } from '../module/event.js'
...
export class ClientTextElement extends ComponentLoader {
  ...
  private async handleClientText(event: Event): Promise<void> {
    ...
    if (isDefiniteError(endpointOrError)) {
      MOVIE_MASHER.dispatchCustom(new EventImporterError(endpointOrError))
      return 
    }
    ...
    MOVIE_MASHER.dispatchCustom(new EventImporterAdd(object))
  }
