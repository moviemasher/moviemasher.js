...
import type { AssetCacheArgs, AssetObject, ChangePropertiesEditObject, ClientAsset, ClientAssets, ClientMashAsset, Clip, ClipIconArgs, ClipObjects, Clips, Edits, EventDispatcherListeners, MashAssetObject, Propertied, PropertyId, PropertyIds, RawType, ScalarsById, Size, Strings, SvgItems, TargetId, TargetIds, Time } from '@moviemasher/shared-lib/types.js'
import type { AddClipsEditObject, AddTrackEditObject, ClipLocation, Masher, MasherArgs, MasherOptions, MoveClipEditObject, RemoveClipEditObject, SelectedProperty, Timeout } from '../types.js'

import { $ASSET, $AUDIO, $CHANGES, $CLIP, $CONTAINER, $CONTENT, $END, $FLIP, $MASH, $PLAY, $VIEW, $ROUND, $VIDEO, DOT, MOVIE_MASHER, SIZE_ZERO, TARGET_IDS, arrayFromOneOrMore, arrayRemove, idGenerate, idGenerateString, idTemporary, isDefiniteError } from '@moviemasher/shared-lib/runtime.js'
import { colorFromRgb, colorRgbDifference, colorToRgb } from '@moviemasher/shared-lib/utility/color.js'
import { isDefined, isPositive } from '@moviemasher/shared-lib/utility/guard.js'
import { assertDefined, assertPopulatedString, assertPositive, isEndpoint, isMashAsset, isPropertyId, isTargetId } from '@moviemasher/shared-lib/utility/guards.js'
...
import { assertClientClip, assertClientMashAsset, isClientClip, isClientMashAsset } from '../guards/ClientMashGuards.js'
import { isClientAsset, isClientAudibleInstance, isClientVisibleInstance } from '@moviemasher/shared-lib/utility/client-guards.js'
import { EventAddAssets, EventAssetId, EventChangeAssetId, EventChangeClipId, EventChangeDragging, EventChangeFrame, EventChangeScalar, EventChangeScalars, EventChangedAssetId, EventChangedClientAction, EventChangedClipId, EventChangedFrames, EventChangedMashAsset, EventChangedPreviews, EventChangedSize, EventChangedTargetIds, EventClip, EventClipId, EventDataType, EventDoClientAction, EventDragging, EventEnabledClientAction, EventFrames, EventManagedAsset, EventManagedAssetPromise, EventMashAsset, EventMashTime, EventMoveClip, EventPreviews, EventPropertyIds, EventRemoveClip, EventScalar, EventSelectable, EventSelectedProperties, EventSize, EventTargetIds, EventTimeRange, EventTrackClipIcon, EventTrackClips, EventWillDestroy } from '../module/event.js'
import { $INSERT, ADD_TRACK, INDEX_CURRENT, INDEX_LAST, INDEX_NEXT, MOVE_CLIP, REDO, $REMOVE, REMOVE_CLIP, UNDO } from './constants.js'
import { endpointUrl } from './request.js'

const Defaults: MasherArgs = { buffer: 10, fps: 30 }
...
class MasherClass implements Masher {
  ...
  private actionIsHandled(action: string): boolean { 
    switch(action) {
      case $INSERT: 
      case ADD_TRACK: 
      case $VIEW:
  ...
  private add() {
    ...
    } else if (time) mashIndex.frame = time.scale(this.quantizeNumber).frame
  ...
  private addAssets(asset: ClientAsset | ClientAssets, mashIndex?: ClipLocation): Promise<ClientAssets> {
    ...
    const installedAssets = arrayFromOneOrMore(asset) 
    ...
    return this.assureMash(installedAssets).then(() => {
      ...
      const options: AddClipsEditObject = {
        clips, type: $INSERT, trackIndex, createTracks, mashAsset
      }
      ...
      if (dense) options.insertIndex = clipIndex
        ...
        if (createTracks) options.redoFrame = clipIndex
          ...
          existingTrack.assureFrames(this.quantizeNumber, clips)
          options.redoFrame = existingTrack.frameForClipNearFrame(firstClip, clipIndex)
        }
      }
      this.edits.create(options)
  ...
  private set assetId(id: string | undefined) {
    ...
    delete this._asset
    this._assetId = id
    MOVIE_MASHER.dispatchCustom(new EventChangedAssetId(id))
    MOVIE_MASHER.dispatchCustom(new EventChangedTargetIds(this.selectedTargetIds))
  }
  ...
  private can(clientAction: string): boolean {
    const { selection: clip, mashAsset: mash, assetId, edits } = this    
    switch (clientAction) {
      case $PLAY: return !!mash?.paused
      case $FLIP:
      case ADD_TRACK: return !!mash
      case $VIEW: return !!mash?.encoding
      case $INSERT: return !!(assetId && mash)
      case UNDO: return edits.canUndo
      case REDO: return edits.canRedo
      case $REMOVE: return !!clip
    }
    return false
  }

  private clipFromId(clipId?: string): Clip | undefined {
    if (!clipId) return
  ...
  private clipIndex(frameOrIndex: number, clips: Clips, dense?: boolean): number {
    ...
    switch (frameOrIndex) {
      ...
      case INDEX_NEXT:
      case INDEX_CURRENT: {
        const { frame } = this.time.scale(this.quantizeNumber)
  ...
  private flip(): void {
    const { mashAsset, edits: actions } = this
    if (!mashAsset) return
  ...
  private get fps(): number {
    return this._fps || this.quantizeNumber || Defaults.fps
  }
  ...
  private dispatchDrawLater(): void {
    ...
    this.drawTimeout = setTimeout(() => {
      MOVIE_MASHER.dispatchCustom(new EventChangedPreviews())
  ...
  private handleClipId(event: EventClipId) {
    event.detail.id = this.selection ? this.selection.id : ''
    event.stopImmediatePropagation()
  }
  ...
  private handleFrame(event: EventMashTime) {
    ...
    const quantize = this.quantizeNumber || this.fps
  ...
  private handleTrackClipIcon(event: EventTrackClipIcon) {
    ...
    if (visible) {
      ...
      args.width = width
      const mashColor = this.mashAsset!.value('color') 
      assertPopulatedString(mashColor, 'mashColor')
  ...
  private handleAssetId(event: EventAssetId) {
    event.stopImmediatePropagation()
    event.detail.id = this.assetId
  }

  private handleChangeAssetId(event: EventChangeAssetId) {
    event.stopImmediatePropagation()
    this.assetId = event.detail
    MOVIE_MASHER.dispatchCustom(new EventChangedClientAction($INSERT))
  ...
  private handleChangeScalar(event: EventChangeScalar) {
    ...
    this.edits.create(actionObject)
    event.stopImmediatePropagation()
  }
  ...
  private handleFrames(event: EventFrames) {    
    ...
    const { detail } = event
    const { mashAsset } = this
    detail.frames = mashAsset?.totalFrames || 0
  }
  ...
  private handleSize(event: EventSize): void {
    ...
    const { detail } = event
    const { mashAsset } = this
    detail.size = mashAsset?.size || SIZE_ZERO
  }
  ...
  load = async (data: AssetObject): Promise<void> => {
    ...
    const { buffer } = this
    const assetObject: MashAssetObject = { ...mashMediaObject, buffer }
  ...
  private set mashAsset(asset: ClientMashAsset | undefined) {
    ...
    if (asset) {
      MOVIE_MASHER.dispatchCustom(new EventChangedSize(asset.size))
      MOVIE_MASHER.dispatchCustom(new EventChangedFrames(asset.totalFrames))
    }
    MOVIE_MASHER.dispatchCustom(new EventChangedMashAsset(asset))
    MOVIE_MASHER.dispatchCustom(new EventChangedClientAction($INSERT))
    MOVIE_MASHER.dispatchCustom(new EventChangedClientAction(ADD_TRACK))
    MOVIE_MASHER.dispatchCustom(new EventChangedClientAction($FLIP))
    MOVIE_MASHER.dispatchCustom(new EventChangedClientAction($PLAY))
  ...
  private move(clip: Clip, editorIndex: ClipLocation = {}): void {
  ...
  private get quantizeNumber(): number { return Number(this.mashAsset?.value('quantize')) || this.fps }

  private redo(): void { 
    if (this.edits.canRedo) this.edits.redo()
  }
  ...
  private removeClip(clientClip?: Clip): void {
  ...
  private _selection: Clip | false = false
  private get selection(): Clip | false { return this._selection }
  private set selection(value: Clip | false) {
    ...
    MOVIE_MASHER.dispatchCustom(new EventChangedClipId(value ? value.id : ''))
    MOVIE_MASHER.dispatchCustom(new EventChangedTargetIds(this.selectedTargetIds))
    MOVIE_MASHER.dispatchCustom(new EventChangedClientAction($REMOVE))
  }

  private selectable(id: TargetId | PropertyId): Propertied | false {
  ...
  private undo(): void {
    const { canUndo } = this.edits
    if (canUndo) this.edits.undo()
  }
