import {
  assertEndpoint, assertPopulatedString, TypeAudio, endpointFromUrl, 
  ...
  promise(request: Request, type?: LoadType) {
    switch (type) {
      ...
      case TypeFont: {
        ...
        const expires = environment.get(EnvironmentKeySupabaseExpires, TypeNumber) || 60
      ...
      case TypeRecord: return this.recordPromise(request)
      case TypeRecords: return this.recordsPromise(request)
...
Runtime.plugins[TypeProtocol][SupabaseProtocol] ||= new ProtocolSupabase()
