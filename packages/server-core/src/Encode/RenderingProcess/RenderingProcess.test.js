...
import { outputDefaultVideo, environment, Environment } from "@moviemasher/server-core"
import { SizePreview, assertPopulatedArray, urlBaseInitialize } from "@moviemasher/moviemasher.js"

import { renderingTestIdPromise, renderingJobPromise } from '../../../../images/tester/Utilities/Rendering.mjs'
...
describe("RenderingProcess", () => {
  ...
  const testFile = async (relative) => {
    ...
    const results = await renderingJobPromise(job, mediaDirectory, userDirectory)
    const [result] = results
    assert(result, 'result')
    const { destination, error } = result
    console.log(result)
