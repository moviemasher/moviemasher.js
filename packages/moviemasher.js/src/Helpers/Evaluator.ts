...
const arrayFromElements = (elements : JsonValue) : Value[] => {
...
const conditionalExpression = (conditional : UnknownObject) : string => {
  ...
  const array = arrayFromElements(<JsonValue> elements)
...
class Evaluator {
  ...
  mm_horz(size : Value, proud : Scalar) : number {
  ...
  mm_vert(size : Value, proud : Scalar) : number {
  ...
  sized(vertical : number, size : Value, proud : Scalar) : number {
