(function (global, factory) {
  ...
  (global = typeof globalThis !== 'undefined' ? globalThis : global || self, global.MovieMasher = factory());
  ...
  const undo = "undo";
  const video = "video";
  const ActionTypes = ['addTrack', 'addClipsToTrack', 'moveClips', 'addEffect', 'change', 'changeFrames', 'changeTrim', 'changeGain', 'moveEffects', 'split', 'freeze', 'removeClips'];
  ...
  const FilterTypes = ["blend", "chromakey", "colorchannelmixer", "color", "convolution", "crop", "drawbox", "drawtext", "fade", "overlay", "scale", "setsar"];
  ...
  class Action {
    ...
    redo() {
      this.redoAction();
      this.done = true;
      this.events && this.events.emit(EventType.action, {
        action: this
      });
    }
    ...
    if (Is.object(ob1)) {
      ...
      for (var key in ob1) {
        if (keyShouldBeCopied(key)) {
          ...
          if (Is.object(value1)) {
            if (Is.array(value1)) ob2[key] = [...value1];else ob2[key] = deepCopy(value1, ob2[key]);
          } else ob2[key] = value1;
        }
      }
    }
  ...
  const lowestCommonMultiplier = (a, b) => {
  ...
  class Time {
    ...
    static scaleTimes(time1, time2, rounding = "round") {
      ...
      return [time1.scale(gcf, rounding), time2.scale(gcf, rounding)];
    }
    ...
    get copy() {
      return new Time(this.frame, this.fps);
    }
    ...
    scale(fps = 1, rounding = "round") {
      if (this.fps === fps) return this; //const scale = Number(this.fps) / Number(fps)
  ...
  class TimeRange extends Time {
    ...
    get endTime() {
      return TimeFactory.create(this.end, this.fps);
    }
    ...
    get startTime() {
      return TimeFactory.create(this.frame, this.fps);
    }
  ...
  const KEYS_GETTERS = ["mm_dimensions", "mm_duration", "mm_fps", "mm_height", "mm_t", "mm_width", "t"];
  const KEYS = ["ceil", "floor", "mm_cmp", "mm_horz", "mm_max", "mm_min", "mm_vert", ...KEYS_GETTERS, ...KEYS_SIZED];
  ...
  function Evaluator(timeRange, context, dimensions) {
    ...
    if (!Is.object(dimensions)) throw Errors.argument + dimensions; // console.log("Evaluator", timeRange, context.constructor.name, dimensions)
  ...
  const conditionalExpression = conditional => {
    const condition = conditional.condition; // not strict equality, since we may have strings and numbers

    if (Is.defined(conditional.is)) return `${condition}==${conditional.is}`;
    const elements = conditional.in;
    if (Is.undefined(elements)) return condition; // support supplying values as array or comma-delimited string
  ...
  Object.defineProperties(Evaluator.prototype, {
    ...
    replaceKeys: {
      value: function (expression) {
        ...
        const expressions = Object.fromEntries(this.keys.map(key => [key, new RegExp(`\\b${key}\\b`, 'g')]));
    ...
    evaluateExpression: {
      value: function (expression) {
        const script = `return ${this.replaceKeys(expression)}`; // console.log("evaluateScope", script)
    ...
    keys: {
      get: function () {
        return [...new Set([...this.map.keys(), ...KEYS])];
      }
    },
    mm_cmp: {
      value: function (a, b, x, y) {
        return a > b ? x : y;
      }
    },
    mm_dimensions: {
      get: function () {
        return `${this.mm_width}x${this.mm_height}`;
      }
    },
    mm_duration: {
      get: function () {
    ...
    mm_t: {
      get: function () {
        return this.timeRange.position;
      }
    },
    mm_vert: {
      value: function (size, proud) {
    ...
    sized: {
      value: function (vertical, size, proud) {
  ...
  class Cache {
    ...
    add(url, value) {
      const key = this.key(url); // console.log("Cache.add", url, key)
    ...
    get(url) {
      const key = this.key(url); // console.log("Cache.get", url, key)
    ...
    urls() {
      return Object.values(this.__urls_by_md5);
    }
  ...
  class Loader extends Base {
    constructor(object) {
      super(object); // console.log("Loader", this.object)
    ...
    loadedUrl(url) {
      return CacheInstance.cached(url);
    }

    loadedUrls(urls) {
      return urls.every(url => this.loadedUrl(url));
    }
  ...
  class AudioProcessor extends Base {
    get audioContext() {
      return this.object.audioContext;
    }

    process(url, buffer) {
      return new Promise((resolve, reject) => {
        return this.audioContext.decodeAudioData(buffer, audioData => resolve(audioData), error => reject(error));
      });
    }
  ...
  class AudioLoader extends Loader {
    ...
    set audioContext(value) {
      this.object.audioContext = value;
    }
  ...
  class UrlsByType {
    ...
    get loaded() {
      return this.urls.every(url => CacheInstance.cached(url));
    }

    get urls() {
      return LoadTypes.flatMap(type => this[type]);
    }

    get urlsUnloaded() {
      return this.urls.filter(url => !CacheInstance.cached(url));
    }
  ...
  class AddEffectAction extends Action {
    ...
    undoAction() {
      Utilities.deleteFromArray(this.effects, this.effect);
    }
  ...
  class AddClipToTrackAction extends AddTrackAction {
    ...
    get track() {
      return this.mash[this.trackType][this.trackIndex];
    }

    redoAction() {
      // create track if needed
      for (let i = 0; i < this.createTracks; i++) {
        super.redoAction();
      }
  ...
  class Actions extends Base {
    ...
    get mash() {
      return this.object.mash;
    } // TODO: remove


    get masher() {
      return this.object.masher;
    }
  ...
  var label$i = "Blackout Two AM";
  var id$j = "com.moviemasher.font.default";
  var type$c = "font";
  var source = "../examples/javascript/media/font/blackout/theleagueof-blackout/webfonts/blackout_two_am-webfont.ttf";
  var family = "Blackout Two AM";
  var fontDefaultJson = {
  	label: label$i,
  	id: id$j,
  	type: type$c,
  ...
  var label$h = "Blur";
  var type$b = "effect";
  var id$i = "com.moviemasher.effect.blur";
  var properties$d = {
  };
  var filters$h = [
  	{
  ...
  var label$g = "Chromakey";
  var type$a = "effect";
  var id$h = "com.moviemasher.effect.chromakey";
  var properties$c = {
  	chroma_blend: {
  		type: "number",
  		value: 0.01
  ...
  var label$f = "Emboss";
  var type$9 = "effect";
  var id$g = "com.moviemasher.effect.emboss";
  var properties$b = {
  };
  var filters$f = [
  	{
  ...
  var label$e = "Grayscale";
  var type$8 = "effect";
  var id$f = "com.moviemasher.effect.grayscale";
  var properties$a = {
  };
  var filters$e = [
  	{
  ...
  var label$d = "Sepia";
  var type$7 = "effect";
  var id$e = "com.moviemasher.effect.sepia";
  var properties$9 = {
  };
  var filters$d = [
  	{
  ...
  var label$c = "Sharpen";
  var type$6 = "effect";
  var id$d = "com.moviemasher.effect.sharpen";
  var properties$8 = {
  };
  var filters$c = [
  	{
  ...
  var label$b = "Text Box";
  var type$5 = "effect";
  var id$c = "com.moviemasher.effect.textbox";
  var properties$7 = {
  	string: {
  		type: "string",
  		value: "Text Box"
  ...
  var label$a = "Blend";
  var id$b = "com.moviemasher.merger.blend";
  var properties$6 = {
  	mode: {
  		type: "mode",
  		value: "normal"
  ...
  var label$9 = "Center";
  var id$a = "com.moviemasher.merger.center";
  var filters$9 = [
  	{
  		id: "overlay",
  		parameters: [
  ...
  var label$8 = "Constrained";
  var type$4 = "merger";
  var id$9 = "com.moviemasher.merger.constrained";
  var properties$5 = {
  	left: {
  		type: "pixel",
  		value: 0
  ...
  var label$7 = "Top Left";
  var id$8 = "com.moviemasher.merger.default";
  var filters$7 = [
  	{
  		id: "overlay",
  		parameters: [
  ...
  var label$6 = "Overlay";
  var id$7 = "com.moviemasher.merger.overlay";
  var properties$4 = {
  	left: {
  		type: "pixel",
  		value: 0.5
  ...
  var label$5 = "Stretch";
  var id$6 = "com.moviemasher.scaler.default";
  var filters$5 = [
  	{
  		id: "scale",
  		parameters: [
  ...
  var label$4 = "Pan";
  var type$3 = "scaler";
  var id$5 = "com.moviemasher.scaler.pan";
  var properties$3 = {
  	scale: {
  		type: "size1",
  		value: 1.25
  ...
  var label$3 = "Scale";
  var type$2 = "scaler";
  var id$4 = "com.moviemasher.scaler.scale";
  var properties$2 = {
  	scale: {
  		type: "size1",
  		value: 1
  ...
  var label$2 = "Color";
  var type$1 = "theme";
  var id$3 = "com.moviemasher.theme.color";
  var properties$1 = {
  	color: {
  		type: "rgb",
  		value: "0xFFFF00"
  ...
  var label$1 = "Title";
  var type = "theme";
  var id$2 = "com.moviemasher.theme.text";
  var properties = {
  	string: {
  		type: "string",
  		value: "Title"
  ...
  class Module {
    constructor() {
      ...
      this[ModuleType.theme] = [themeColorJson, themeTextJson];
      this[ModuleType.effect] = [effectBlurJson, effectChromaKeyJson, effectEmbossJson, effectGrayscaleJson, effectSepiaJson, effectSharpenJson, effectTextJson];
      this[ModuleType.font] = [this.defaults.font];
      this[[ModuleType.merger]] = [mergerBlendJson, mergerCenterJson, mergerConstrainedJson, mergerOverlayJson, this.defaults.merger];
      this[ModuleType.scaler] = [scalerPanJson, scalerScaleJson, this.defaults.scaler];
    }
  ...
  const id$1 = {
    id: {
      get: function () {
        return this.__id ||= this.initializeId;
      }
    },
    initializeId: {
      get: function () {
        return this.object.id || Id();
      }
    }
  };
  ...
  const object = {
    property: {
      value: function (key) {
        return this[key] || this.object[key];
      }
    }
  };
          ...
          const entries = names.map(name => [name, {
            get: function () {
              ...
              if (Is.undefined(this[key])) {
                ...
                this[key] = PropertyInstance.orDefault(property); //console.log("initialized", key, this[key])
              }
  ...
  class ChromaKeyFilter extends CoreFilter {
    ...
    draw(evaluator, evaluated) {
      ...
      const yuvs = accurate ? this.yuvsFromPixelsAccurate(pixels, width, height) : this.yuvsFromPixels(pixels);
      let offset = 0;
      yuvs.forEach(matrix => {
  ...
  class DrawTextFilter extends CoreFilter {
    ...
    draw(evaluator, evaluated) {
      ...
      if (shadowcolor) {
        ...
        } = evaluated; // shadowblur not supported in ffmpeg 
      ...
      if (shadowcolor) {
        ...
        context.shadowOffsetY = 0; // context.shadowBlur = 0 
      }
  ...
  class ScaleFilter extends CoreFilter {
    draw(evaluator, evaluated) {
      ...
      const in_width = evaluator.get("mm_input_width");
      const in_height = evaluator.get("mm_input_height");
      if (-1 === width) width = in_width * (height / in_height);else if (-1 === height) height = in_height * (width / in_width);
      const drawing = ContextFactoryInstance.createDrawing(width, height);
  ...
  const drawFilters = {
    drawFilters: {
      value: function (evaluator, context) {
  ...
  const filters = {
    filters: {
      get: function () {
        return this.__filters ||= this.filtersInitialize;
      }
    },
    filtersInitialize: {
      get: function () {
      ...
      value: function (timeRange, clip) {
        ...
        Object.keys(properties_by_type).forEach(type => {
          ...
          modular_properties.forEach(property => {
            const value = clip.property(property); // console.log(clip)
  ...
  const transform$1 = { ...filters,
    ...urlsFromFilters,
    ...html,
    ...modular
  };

  function EffectMedia(object) {
    this.object = object;
  }
  ...
  function ScalerMedia(object) {
    this.object = object;
  }

  Object.defineProperties(ScalerMedia.prototype, {
    type: {
      value: MediaType.scaler
    },
    ...sharedMedia,
    ...transform$1,
    ...inaudible$1,
  ...
  function MergerMedia(object) {
    this.object = object;
  }

  Object.defineProperties(MergerMedia.prototype, {
    type: {
      value: MediaType.merger
    },
    ...sharedMedia,
    ...transform$1,
    ...inaudible$1,
  ...
  function FontMedia(object) {
    this.object = object;
  }

  Object.defineProperties(FontMedia.prototype, {
    type: {
      value: MediaType.font
    },
    ...sharedMedia,
    ...filters,
    ...urlsFromFilters,
  ...
  const urlsVisibleFrames = {
    fps: {
      get: function () {
        return this.__fps ||= this.object.fps || 10;
      }
    },
    framesMax: {
      get: function () {
    ...
    begin: {
      get: function () {
        return this.__begin ||= this.object.begin || 1;
      }
    },
    pattern: {
      get: function () {
        return this.__pattern ||= this.object.pattern || '%.jpg';
      }
    },
    increment: {
      get: function () {
        return this.__increment ||= this.object.increment || 1;
      }
    },
    zeropadding: {
      get: function () {
    ...
    url: {
      get: function () {
        return this.__url ||= this.object.url || "";
      }
    },
    limitedTimeRange: {
    ...
    urlsVisibleInTimeRangeForClipByType: {
      value: function (timeRange, clip) {
        ...
        for (let frame = range.frame; frame < range.end; frame++) {
          ...
          const media_time = time.scale(this.fps); //console.log(time, media_time)
  ...
  function TransitionMedia(object) {
    this.object = object;
  }

  Object.defineProperties(TransitionMedia.prototype, {
    type: {
      value: MediaType.transition
    },
    ...sharedMedia,
    ...modular,
    ...inaudible$1,
  ...
  const createFromTime = (time, frames) => createFromArgs(time.frame, time.fps, frames);
  ...
  const id = {
    id: {
      get: function () {
        return this.object.id || this.media.id;
      }
    }
  };
  ...
  const labelFromObjectOrMedia = {
    label: {
      get: function () {
        return this.__label ||= this.labelInitialize;
      },
      set: function (value) {
        this.__label = value;
      }
    ...
    labelInitialize: {
      get: function () {
        return this.object.label || this.media.label;
      }
    }
  };
  ...
  const editable = { ...object,
    ...
    properties: {
      get: function () {
        return this.__properties ||= this.propertiesInitialize;
      }
    },
    propertiesInitialize: {
      get: function () {
  ...
  const visible = {
    ...
    scaledContextAtTimeForDimensions: {
      value: function (mashTime, dimensions, ...rest) {
        const context = this.contextAtTimeForDimensions(mashTime, dimensions, ...rest); // console.log("scaledContextAtTimeForDimensions", context)
    ...
    effectedContextAtTimeForDimensions: {
      value: function (mashTime, dimensions, ...rest) {
        ...
        const clipTimeRange = this.timeRangeRelative(mashTime);
        if (Is.undefined(clipTimeRange)) return;
        this.effects.reverse().forEach(effect => context = effect.drawMediaFilters(clipTimeRange, context, dimensions));
        return context;
      }
    },
    mergeContextAtTime: {
  ...
  const urls = {
    urlsVisibleInTimeRangeByType: {
      value: function (mashTimeRange) {
        ...
        if (this.effects) this.effects.forEach(effect => urls.concat(effect.urlsVisibleInTimeRangeByType(range)));
        return urls;
      }
    }
  };
  ...
  const idMutable = {
    id: {
      get: function () {
        return this.__id ||= this.object.id;
      },
      set: function (value) {
        this.__id = value;
        this.__media = null;
  ...
  const drawMerger = {
    drawMerger: {
      value: function (clipTimeRange, inputContext, outputContext) {
  ...
  const sharedTransform = { ...editable,
    ...copy,
    ...media,
    ...labelFromObjectOrMedia,
    ...propertyValues,
  ...
  Object.defineProperties(Transform.prototype, { ...sharedTransform
  });
  ...
  const drawMediaFilters = {
    drawMediaFilters: {
      value: function (clipTimeRange, context, dimensions) {
  ...
  const transform = {
    effects: {
      get: function () {
        return this.__effects ||= this.effectsInitialize;
      }
    },
    effectsInitialize: {
      get: function () {
    ...
    merger: {
      get: function () {
        return this.__merger ||= this.mergerInitialize;
      },
      set: function (value) {
        this.__merger = new Merger({
          id: value
        });
      }
    },
    ...
    scaler: {
      get: function () {
        return this.__scaler ||= this.scalerInitialize;
      },
      set: function (value) {
        this.__scaler = new Scaler({
          id: value
        });
      }
    },
  ...
  const audible = {
    ...
    mediaTimeRange: {
      value: function (timeRange) {
        ...
        return TimeRangeFactory.createFromTimes(this.mediaTime(timeRange.startTime), this.mediaTime(timeRange.endTime, add_one_frame));
      }
    }
  };
  ...
  function FrameClip(object) {
    this.object = object;
  }

  Object.defineProperties(FrameClip.prototype, {
    type: {
      value: ClipType.frame
    },
    ...sharedClip,
    ...visible,
    ...urls,
  ...
  class Track extends Base {
    ...
    get clips() {
      return this.__clips ||= this.initializeClips;
    }
    ...
    get initializeClips() {
      ...
      return array.map(object => {
        ...
        const clip = ClipFactoryInstance.create({
          track: this.index,
          ...object
        }, media);
        return clip;
    ...
    get isMainVideo() {
      return !this.index && this.type == TrackType.video;
    }
    ...
    addClips(clips, insertIndex = 0) {
      ...
      const spliceClips = this.clips.filter((clip, index) => {
        ...
        if (moving) movingClips.push(clip); // insert index should be decreased when clip is moving and comes before
      ...
      this.sortClips(spliceClips); // set the track of clips we aren't moving 

      const newClips = clips.filter(clip => !movingClips.includes(clip));
      newClips.forEach(clip => clip.track = this.index); // remove all my current clips and replace with new ones in one step
  ...
  class TrackFactoryClass extends Factory {
    create(object, mash) {
      if (!Is.object(object)) throw Errors.object;
      if (object instanceof Track) return object; //if (!Is.instance(mash, Mash)) throw(Errors.mash)
  ...
  class Mash extends Base {
    static get properties() {
      return Object.values(MashProperty);
    }

    get audio() {
      return this.__audio ||= this.initializeAudio;
    }
    ...
    get clipsAudible() {
      return this.clips.filter(clip => clip.audible);
    }

    get clipsAudio() {
      return this.audio.map(track => track.clips).flat();
    }

    get clipsVideo() {
      return this.video.flatMap(track => track.clips);
    }

    get events() {
      return this.__events ||= this.object.events;
    }
    ...
    get initialMedia() {
      return this.__initialMedia ||= this.initializeMedia;
    }

    get label() {
      return this.__label ||= this.object.label || Default.mash.label;
    }
    ...
    get quantize() {
      return this.__quantize ||= this.object.quantize || Default.mash.quantize;
    }
    /** combined audio and video tracks */


    get tracks() {
      return TrackTypes.map(av => this[av]).flat();
    }
    ...
    get video() {
      return this.__video ||= this.initializeVideo;
    }

    addClipsToTrack(clips, trackIndex = 0, insertIndex = 0) {
      trackIndex ||= 0;
      insertIndex ||= 0;
      const [clip] = clips; // console.log("addClipsToTrack trackIndex", trackIndex, "insertIndex:", insertIndex)
    ...
    clipsAudibleAtTime(time) {
      return this.clipsWithin(time, true, false);
    }
    ...
    clipsVisibleAtTime(time) {
      return this.clipsWithin(time, false);
    }
    ...
    emitIfFramesChange(method) {
      const frames = this.events ? this.frames : null; // console.log(this.constructor.name, "emitIfFramesChange", frames)

      method();
      if (this.events && frames !== this.frames) this.emitDuration(); // else console.log(this.constructor.name, "emitIfFramesChange", !!this.events, frames, this.frames)
    }
    ...
    filterClipSelection(value) {
      ...
      const [firstClip] = clips; // console.log("filterClipSelection", clips)

      if (!firstClip) return [];
      const {
        trackType,
        track
      } = firstClip; // selected clips must all be on same track

      const trackClips = clips.filter(clip => clip.track === track && clip.trackType === trackType).sort(Sort.byFrame);
      if (track || trackType === TrackType.audio) return trackClips; // selected clips on main track must be abutting each other
  ...
  Object.defineProperties(Mash.prototype, { ...id$1
  });
  ...
  const deprecated = {
    ...
    did: {
      value: function (_removedCount) {// angular-moviemasher overrides this to be alerted when we actions that
  ...
  class Composition extends Base {
    ...
    get audible() {
      return !this.paused && !!this.audioContext;
    }

    get bufferedTime() {
      return this.urlsAtTime.loaded;
    }
    ...
    get bufferSeconds() {
      return this.object.bufferSeconds;
    }

    set bufferSeconds(value) {
      this.object.bufferSeconds = value;
    }
    ...
    get configured() {
      return this.time && this.mash && (this.videoContext || this.audioContext);
    }
    ...
    set mash(value) {
      this.object.mash = value;
    }

    get quantize() {
      return this.mash ? this.mash.quantize : 1;
    }
    ...
    get timeRangeAtTime() {
      return TimeRangeFactory.createFromTime(this.time);
    }
    ...
    adjustSourceGain(clip) {
      ...
      if (!Is.array(gain)) return gainNode.gain.value = gain * this.gain; // time/gain pairs...
  ...
  Object.defineProperties(Composition.prototype, { ...contexts
  });
  ...
  const clipCanBeSplit = (clip, time, quantize) => {
    if (!Is.object(clip)) return; // true if now intersects clip time, but is not start or end frame

    let range = TimeRangeFactory.createFromTime(time);
    const clip_range = clip.timeRange(quantize); // ranges must intersect
  ...
  class Masher extends Base {
    ...
    static get properties() {
      return Object.values(MasherProperty);
    }
    ...
    constructor(object = {}) {
      ...
      this.events.addListener(this.handleMasher.bind(this)); // console.log("events:", this.events)
    ...
    get bufferedTimeRange() {
      return this.composition.bufferedTimeRange;
    }

    get canvas() {
      return this.videoContext.canvas;
    }
    ...
    set currentTime(seconds) {
      this.time = TimeFactory.createFromSeconds(seconds, this.fps);
    }

    get duration() {
      return TimeFactory.create(this.__mashFrames, this.mash.quantize).seconds;
    }

    get endTime() {
      return this.mashEndTime.scale(this.fps, 'floor');
    }
    ...
    set frame(value) {
      ...
      this.time = TimeFactory.create(value, this.fps); //console.log("time", this.time)
    } // Math.round(Number(this.__mashFrames) / Number(this.mash.quantize) * Number(this.fps));}


    get frames() {
      return this.mashEndTime.scale(this.fps).frame;
    }
    ...
    get mash() {
      return this.__mash ||= this.object.mash;
    }
    ...
    get mashEndTime() {
      return TimeFactory.create(this.__mashFrames, this.mash.quantize);
    }
    ...
    set position(value = 0) {
      this.time = TimeFactory.createFromSeconds(this.duration * value, this.fps);
    }
    ...
    set selectedClip(value) {
      this.selectedClips = value;
    }

    get selectedClipOrMash() {
      return this.selectedClip || this.__mash;
    }

    get selectedClips() {
      return this.__selectedClips;
    }
    ...
    set selectedEffects(value) {
      ...
      if (!effects) {
        // mash or multiple clips selected, or no effects
        this.__selectedEffects = [];
    ...
    get silenced() {
      return this.__paused || this.muted || !this.gain;
    }

    get stalling() {
      return !this.__moving && !this.paused;
    }
    ...
    get urlsInUse() {
      return this.composition.urlsInTimeRange.urls;
    }
    ...
    changeClip(property) {
      ...
      switch (options.property) {
        ...
        default:
          options.type = ActionType.change;
      }
    ...
    handleGainChange() {
      this.composition.gain = this.gain;
    }
    ...
    handleChangeMashFrames() {
      ...
      if (this.__mashFrames !== mash_frames) {
        this.__mashFrames = mash_frames; // move back if we were positioned at a time later than new duration

        const max_frame = Math.max(0, this.endTime.frame - 1);
        if (max_frame < this.time.frame) this.frame = max_frame;else this.delayedDraw();
      }
    }

    handleMasher(event) {
      if (event.type !== Events.type) return console.warn("handleMasher", event); // console.log("handleMasher", event.detail)
    ...
    handleRemoveActions(removedActions = []) {
      ...
      this.handleChangeMash(); // TODO: deprecated - stop calling and remove did method
    ...
    moveEffects(effectOrArray, index = 0) {
      ...
      if (!Is.positive(index)) throw Errors.argument + index; // Coerce.instanceArray? 
    ...
    redraw() {
      ...
      const audio_buffered = no_audio || this.composition.bufferedTime; // console.log(this.constructor.name, "redraw audio_buffered:", audio_buffered)

      const all_buffered = video_buffered && audio_buffered;

      if (this.__moving !== all_buffered) {
        if (this.__moving) this.__set_moving(false);else if (!this.__paused && this.bufferedTimeRange) this.__set_moving(true);
      }
    }

    draw() {
      return this.composition.draw();
    }
    ...
    select(object, toggle_selected) {
      ...
      var media,
          i,
          array = [],
          key_array = '__selectedClips',
          key_prop = 'selectedClips';
    ...
    selectEffect(effect, toggle_selected) {
      if (Is.object(effect)) return this.select(effect, toggle_selected);else if (!toggle_selected) this.selectedEffect = effect;
    }
    ...
    __limit_time(time) {
      ...
      return scaled; // console.log("__limit_time", time)
    ...
    __set_moving(tf) {
      ...
      if (this.__moving !== tf) {
        ...
        if (this.__moving) {
          var $this = this;
          this.__load_timer = setInterval(() => {
            $this.__load_timed();
          }, 500 / this.fps); // start up the sound buffer with our current time, rather than displayed
  ...
  Object.defineProperties(Masher.prototype, { ...contexts,
    ...deprecated
  });
  ...
  class CacheCleaner {
    ...
    handleInterval() {
      ...
      Masher.instances.forEach(masher => {
        const urls = masher.urlsInUse; // console.log("CacheCleaner.handleInterval urls:", urls)
      ...
      remove_urls.forEach(url => CacheInstance.remove(url)); // const count = remove_urls.length
  ...
  MovieMasher.configure = object => {
    if (Is.object(object)) console.warn(Errors.deprecation.configure.set);else console.warn(Errors.deprecation.configure.get);
    return Default;
  };
  ...
  Object.defineProperties(MovieMasher, {
    initialize: {
      value: function () {
        console.log("MovieMasher.initialize");
      }
    },
    MovieMasher: {
      get: function () {
